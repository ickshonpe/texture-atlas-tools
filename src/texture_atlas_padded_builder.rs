...
/// sprites.
pub struct TextureAtlasPaddedBuilder {
    ...
    /// amount of padding to add to right and bottom of the texture rects
    padding: [u32; 2],
}

impl Default for TextureAtlasPaddedBuilder {
    fn default() -> Self {
        Self {
...
impl TextureAtlasPaddedBuilder {
    ...
    pub fn add_texture(&mut self, texture_handle: Handle<Image>, texture: &Image) {
        self.rects_to_place.push_rect(
            ...
            None,
            RectToInsert::new(
                texture.texture_descriptor.size.width + self.padding[0],
                texture.texture_descriptor.size.height + self.padding[1],
                1,
            ),
        )
    }
    ...
    pub fn finish(
        ...
        for (texture_handle, (_, packed_location)) in rect_placements.packed_locations().iter() {
            ...
            let max = min
                + Vec2::new(
                    (packed_location.width() - self.padding[0]) as f32,
                    (packed_location.height() - self.padding[1]) as f32,
                );
